...
module.exports = function Login(util, events){
                    ...
                    Step(
                        ...
                        function sumUp(err, results){
                            if(err){
                                ...
                                that.emit('getUsersOnTheSameRoutes', {data: data.data, routes: data.routes, user_id: user_id});
                            }
                        }
                    )
            });
    ...
    var _getUsersOnTheSameRoutes = function(data){
        ...
        data.data.mysql.query(
            'SELECT routes_users.routes_id, users.trainshare_id FROM routes_users, users WHERE routes_users.users_id = users.id AND routes_users.routes_id IN (?) AND routes_users.users_id != ?',
            [routes_or_clause, data.user_id],
            function(err, results, fields){
                if(err){
                    console.log(err);
                    data.data.response.json({error: 'Internal Error'}, 500);
                    ...
                    console.log(results);
                    that.emit('formatResponse', {data: data.data, routes: data.routes, user_id: data.user_id});
