...
module.exports = function Login(util, events){
            ...
            function(err, results, fields){
                if(err){
                    ...
                    if(results.length === 0){
                        ...
                        // TODO -> change trainshare_token -> write to DB
                        data.response.json({
                            trainshare_id: results[0].trainshare_id, 
                            trainshare_token: results[0].trainshare_token
                        }, 200);
                    }
                }
            });
    ...
    var _fetchTwitterFollowers = function(data){
        ...
        data.twit.getFollowersIds(parseInt(data.twitter_user.id_str), function(followers){
            if(typeof followers !== 'undefined' && followers.length > 0){
                data.twitter_followers = followers; // ['id_1', 'id_2', 'id_3', ...]

                for(var i = 0; i < followers.length; i++){

                    that.emit('checkIfTwitterFollowerIsInDB', {data: data, twitter_followers_index: i});

                    // (function(index, new_data){

                    //     new_data.twitter_follower = index;
                    //     that.emit('checkIfTwitterFollowerIsInDB', new_data);

                    // })(i, data);

                    // function hold_value(value){
                    //     data.twitter_follower = value;
                    //     that.emit('checkIfTwitterFollowerIsInDB', data);
                    //     // that.emit('checkIfTwitterFollowerIsInDB', {data: data, twitter_follower: i});
                    // }(i);

                    // data.twitter_follower = i;
                    // (function(i){
                    //     data.twitter_follower = i;
                    //     that.emit('checkIfTwitterFollowerIsInDB', data);
                    // })(i);
    ...
    var _checkIfTwitterFollowerIsInDB = function(data){
        ...
        // INFO :: only users who've signed up for trainshare have a trainshare_id in the mysql DB.

        data.data.mysql.query(
            'SELECT * FROM users WHERE twitter_uid = ? LIMIT 1',
            [data.data.twitter_followers[data.twitter_followers_index]],
            function(err, results, fields){
                if(err){
                    console.log(err);
                } else {
                    if(results.length === 0){
                        that.emit('insertTwitterFollowerIntoMysql', data.data);
                    } else {
                        // check if already in relationship with root_node in Neo4j
                    }
                }
        });
    }

    // not implemented yet -> meaning no connected to the emits hooks yet.
